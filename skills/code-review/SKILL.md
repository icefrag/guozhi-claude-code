...
**Establish the review target first.** The target is this round's work: local commits not yet pushed plus uncommitted changes — pushed work is already reviewed and out of scope. Resolve in this order, stopping at the first that applies:

1. **Current branch has an upstream:** `git diff @{u}...HEAD` plus uncommitted changes (`git diff` / `git diff --cached`).
2. **No upstream (an unpushed feature branch or worktree):** `git diff <base>...HEAD` against the repo's main development branch, plus uncommitted changes.
3. **Not a git repository:** ask the user which files to review.
...
**Every finding must cite a specific file:line reference and a concrete fix recommendation.** A finding the author can't locate or act on isn't a finding — don't report it.
